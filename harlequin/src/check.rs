...
fn infer<A: Span>(env: &TypeEnv, subst: &mut Subst, e: &Expr<A>) -> Result<Ty, Error> {
    // env contains the list of types we already know, subst contains the substitutions we have and
    // e is the expression whose type we want to know.
    let res = match &e.kind {
        Num(_) | Input => Ty::Int, // Num and input are always int
        True | False => Ty::Bool,  // True and false are always bool
        Add1(e) | Sub1(e) | Neg(e) => infer_app(
            // Add1, Sub1 and Neg are a poly type function
            // which take int and return int
            &e.ann,
            env,
            subst,
        ...
        Vek(e1, e2) => {
            println!("Trying to work with a vec mayhaps");
            infer_app(
                &e.ann,
                env,
                subst,
        ...
        Call(f, es) => {
            println!("Your function call sire");
            let f_sig = env.lookup(f)?; // If not found will it throw an error?
        ...
        Fun(defn) => {
            let sig_inferred = infer_sig(env, subst, defn)?;
...
fn infer_sig<A: Span>(env: &TypeEnv, subst: &mut Subst, defn: &Defn<A>) -> Result<Ty, Error> {
    match &defn.sig {
        Sig::Assume(poly, _) => Ok(subst.instantiate(poly)), // If it is Assumed to be poly,
        // instantiate the generalized vars
        Sig::Infer => infer_defn(env, subst, defn), // If we need to infer, call infer_defn
        Sig::Check(poly, ann) => check_defn(ann, env, subst, defn, poly), // If we need to check
                                                     // call check_defn
    }
}
...
fn infer_defn<A: Span>(env: &TypeEnv, subst: &mut Subst, defn: &Defn<A>) -> Result<Ty, Error> {
    println!("Inside the infer defn function");
    let (in_tys, out_ty) = fresh_fun(defn, subst);
    println!("About to go inside infer_defn_with");
    infer_defn_with(env, subst, defn, in_tys, out_ty)
    //todo!("TBD: fill this in")
}

fn fresh_fun<A: Span>(defn: &Defn<A>, subst: &mut Subst) -> (Vec<Ty>, Ty) {
    let in_tys: Vec<Ty> = defn
        .params
        .iter()
        .map(|_| subst.fresh())
        .collect::<Vec<_>>();
...
fn infer_defn_with<A: Span>(
    ...
    if let Some(f) = &defn.name {
        let t = fun(in_tys.clone(), out_ty.clone());
        binds.push((f.clone(), mono(t.clone())));
    }
...
fn infer_app<A: Span>(
    ...
    println!("The mono is  {:?}", mono);
    println!("The t2 is {:?}", t2);
...
fn unify<A: Span>(ann: &A, subst: &mut Subst, t1: &Ty, t2: &Ty) -> Result<(), Error> {
    match (t1, t2) {
        ...
        (Ty::Fun(ins1, out1), Ty::Fun(ins2, out2)) => {
            println!("Unifying two functions");
        ...
        (Ty::Vec(s1, s2), Ty::Fun(ins, out)) => Err(Error::new(
            ann.span(),
            format! {"You are trying to unify a vec and a fun {t1} and {t2}"},
        )),
        (_, _) => Err(Error::new(
            ann.span(),
...
fn poly_sig(op: Prim) -> Poly {
    match op {
        ...
        Prim::AToA => forall(vec![tv("a")], fun(vec![Ty::Var(tv("a"))], Ty::Var(tv("a")))),
        Prim::AAToBool => forall(
            vec![tv("a")],
            fun(vec![Ty::Var(tv("a")), Ty::Var(tv("a"))], Ty::Bool),
        ),
        Prim::BoolAAToA => forall(
            vec![tv("a")],
            fun(
                vec![Ty::Bool, Ty::Var(tv("a")), Ty::Var(tv("a"))],
                Ty::Var(tv("a")),
            ),
        ),
        Prim::AAToA => forall(
            vec![tv("a")],
            fun(vec![Ty::Var(tv("a")), Ty::Var(tv("a"))], Ty::Var(tv("a"))),
        ),
        Prim::ABToVecAB => forall(
            vec![tv("a"), tv("b")],
            fun(
                vec![Ty::Var(tv("a")), Ty::Var(tv("b"))],
                Ty::Vec(Box::new(Ty::Var(tv("a"))), Box::new(Ty::Var(tv("b")))),
            ),
        ),
    }
}
