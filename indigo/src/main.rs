...
fn compile_expr(e: &Expr, env: &Alloc, count: &mut i32, brk: &str, dst: &Loc) -> String {
    match e {
        ...
        Expr::Input => {
            // Move into destination the prog representation of the value
            // For vars make sure it is not a mem to mem move
            // Easiest to do mov into rax and mov rax into destination
            *count += 1;
            format!(
                "mov rax, [rbp - 8]
                     mov {dst}, rax"
        ...
        Expr::Add1(i) => {
            // i could be a variable or a number, if it is a variable get the stack spot
            // match dst
            // Reg(r) => Move into r and inc
            // Stack(i) => Mov rax {i} -> Add rax 2 -> Mov rax into stack spot

            *count += 1;
            let imm = imm_to_arg(i, env);
            format!(
                "mov rax, {imm}
                 add rax, 2
                 mov {dst}, rax"
        ...
        Expr::Mult(i1, i2) => {
            // Same as above
            *count += 1;
            let imm1 = imm_to_arg(i1, env); // This is in the prog expr
            let imm2 = imm_to_arg(i2, env);
            format!(
                "mov rax, {imm1}
                 shr rax, 1
        ...
        Expr::Let(x, e1, e2) => {
            // The reg_alloc function already tells us where x is supposed to be
            // Compile e1 and save into this location
            // Compile e2 with the updated alloc and save that into the destination
            *count += 1;
            let var_pos = var_to_arg(env, x);
            let var_pos = match var_pos {
                Arg::Var(loc, _) => loc,
                _ => panic!("Invalid variable location given!"),
            };
            let e1_code = compile_expr(e1, env, count, brk, &Loc::Reg(Reg::RAX));
            let e2_code = compile_expr(e2, env, count, brk, dst);
            format!(
                "
                    {e1_code}
                    mov {var_pos}, rax
