...
fn compile(name: &str, file: &Path) -> Result<(), String> {
    ...
    let path = format!("{}/{}", file.parent().unwrap().to_string_lossy(), name);
    let output = Command::new(&compiler)
        .arg(file)
        .arg(&mk_path(&path, Ext::Asm))
        .output()
...
fn run(file: &Path, name: &str, input: Option<&str>) -> Result<String, String> {
    let path = format!("{}/{}", file.parent().unwrap().to_string_lossy(), name);
    let mut cmd = Command::new(&mk_path(&path, Ext::Run));
