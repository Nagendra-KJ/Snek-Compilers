        ...
        Sexp::List(vec) => {
            ...
            match &vec[..] {
                ...
                [Sexp::Atom(S(op)), elems @ ..] if op == "vec" => {
                    let elems_expr = elems
                        .iter()
                        .map(|e| parse_expr(e, break_stack, defn_map))
                        .collect();
                    Expr::Vec(elems_expr)
                }
                [Sexp::Atom(S(op)), vector, idx] if op == "vec-get" => {
                    let vec_exp = parse_expr(vector, break_stack, defn_map);
                    let idx_exp = parse_expr(idx, break_stack, defn_map);
...
fn compile_expr(
    ...
    match e {
        ...
        Expr::DefFun(fname, exp, num_args, rsp_offset) => {
            ...
            if *fname == MAIN_FUN {
                // We also need to save call preserved R12 register before main
                // is called
            ...
            x86_insts.push(Op::Offset(Reg::Rsp, *rsp_offset));
            if *fname == MAIN_FUN {
                // The main function has the args in rdi and rsi
        ...
        Expr::Vec(elems) => {
            for (idx, elem) in elems.iter().enumerate() {
                ...
                compile_expr(elem, x86_insts, env, sp + idx + 1, curr_fun, false);
                x86_insts.push(Op::Mov(
        ...
        Expr::VecGet(vec_exp, idx_exp) => {
            compile_expr(vec_exp, x86_insts, env, sp, curr_fun, false);
            ...
            compile_expr(idx_exp, x86_insts, env, sp + 1, curr_fun, false);
    ...
    match s {
        Sexp::List(vec) => match &vec[..] {
            [Sexp::Atom(S(op)), Sexp::List(fargs), defn] if op == "fun" => {
                if fargs.len() < 1 {
                    panic!("Invalid function name given! A function must have a name");
                }
                ...
                let fparams = &fargs[1..];
                let param_list: Vec<_> = fparams.iter().map(|param| param.to_string()).collect();
                for param in param_list.iter() {
                    if is_keyword(param) {
                        panic!(
                            "Invalid function argument name given! Argument name matches keyword"
                        );
                    }
                }
